...
pkgdatadir = $(datadir)/system-config-printer
pkgincludedir = $(includedir)/system-config-printer
pkglibdir = $(libdir)/system-config-printer
pkglibexecdir = $(libexecdir)/system-config-printer
...
PRE_UNINSTALL = :
POST_UNINSTALL = :
#udev_PROGRAMS = udev/udev-configure-printer$(EXEEXT)
subdir = .
...
am__installdirs = "$(DESTDIR)$(udevdir)" "$(DESTDIR)$(bindir)" \
	...
	"$(DESTDIR)$(cupshelpersdir)" "$(DESTDIR)$(dbusdir)" \
	"$(DESTDIR)$(dbusinterfacesdir)" \
...
#am_udev_udev_configure_printer_OBJECTS = udev/udev_udev_configure_printer-udev-configure-printer.$(OBJEXT)
udev_udev_configure_printer_OBJECTS =  \
	$(am_udev_udev_configure_printer_OBJECTS)
am__DEPENDENCIES_1 =
#udev_udev_configure_printer_DEPENDENCIES =  \
#	$(am__DEPENDENCIES_1) $(am__DEPENDENCIES_1)
udev_udev_configure_printer_LINK = $(CCLD) \
	$(udev_udev_configure_printer_CFLAGS) $(CFLAGS) $(AM_LDFLAGS) \
	$(LDFLAGS) -o $@
...
am__v_CCLD_1 = 
SOURCES = $(udev_udev_configure_printer_SOURCES)
DIST_SOURCES = $(am__udev_udev_configure_printer_SOURCES_DIST)
...
DATA = $(appdata_DATA) $(cupshelpers_DATA) $(dbus_DATA) \
	$(dbusinterfaces_DATA) $(dbusservices_DATA) $(desktop_DATA) \
	$(nobase_pkgdata_DATA) $(systemdsystemunit_DATA) \
	$(udevrules_DATA)
...
ACLOCAL = ${SHELL} /home/wufeiyun/git/system-config-printer-1024/missing aclocal-1.15
...
AUTOCONF = ${SHELL} /home/wufeiyun/git/system-config-printer-1024/missing autoconf
AUTOHEADER = ${SHELL} /home/wufeiyun/git/system-config-printer-1024/missing autoheader
AUTOMAKE = ${SHELL} /home/wufeiyun/git/system-config-printer-1024/missing automake-1.15
AWK = gawk
CATOBJEXT = .gmo
CC = gcc
...
CYGPATH_W = echo
DATADIRNAME = share
DEFS = -DPACKAGE_NAME=\"system-config-printer\" -DPACKAGE_TARNAME=\"system-config-printer\" -DPACKAGE_VERSION=\"1.5.8\" -DPACKAGE_STRING=\"system-config-printer\ 1.5.8\" -DPACKAGE_BUGREPORT=\"\" -DPACKAGE_URL=\"\" -DPACKAGE=\"system-config-printer\" -DVERSION=\"1.5.8\"
DEPDIR = .deps
DESKTOPPREFIX = 
DESKTOPVENDOR = 
ECHO_C = 
...
LIBS = 
LTLIBOBJS = 
MAKEINFO = ${SHELL} /home/wufeiyun/git/system-config-printer-1024/missing makeinfo
MKDIR_P = /bin/mkdir -p
...
PACKAGE_STRING = system-config-printer 1.5.8
...
abs_builddir = /home/wufeiyun/git/system-config-printer-1024
abs_srcdir = /home/wufeiyun/git/system-config-printer-1024
abs_top_builddir = /home/wufeiyun/git/system-config-printer-1024
abs_top_srcdir = /home/wufeiyun/git/system-config-printer-1024
ac_ct_CC = gcc
am__include = include
am__leading_dot = .
am__quote = 
...
install_sh = ${SHELL} /home/wufeiyun/git/system-config-printer-1024/install-sh
...
pdfdir = ${docdir}
pkgpyexecdir = ${pyexecdir}/system-config-printer
pkgpythondir = ${pythondir}/system-config-printer
prefix = /test
...
$(am__aclocal_m4_deps):
system-config-printer: $(top_builddir)/config.status $(srcdir)/system-config-printer.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
system-config-printer-applet: $(top_builddir)/config.status $(srcdir)/system-config-printer-applet.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
install-printerdriver: $(top_builddir)/config.status $(srcdir)/install-printerdriver.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
...
udev/udev-configure-printer$(EXEEXT): $(udev_udev_configure_printer_OBJECTS) $(udev_udev_configure_printer_DEPENDENCIES) $(EXTRA_udev_udev_configure_printer_DEPENDENCIES) udev/$(am__dirstamp)
	@rm -f udev/udev-configure-printer$(EXEEXT)
	$(AM_V_CCLD)$(udev_udev_configure_printer_LINK) $(udev_udev_configure_printer_OBJECTS) $(udev_udev_configure_printer_LDADD) $(LIBS)
...
install-nobase_pkgdataSCRIPTS: $(nobase_pkgdata_SCRIPTS)
	@$(NORMAL_INSTALL)
	@list='$(nobase_pkgdata_SCRIPTS)'; test -n "$(pkgdatadir)" || list=; \
	if test -n "$$list"; then \
	...
	while read type dir files; do \
	  case $$type in \
	  d) echo " $(MKDIR_P) '$(DESTDIR)$(pkgdatadir)/$$dir'"; \
	     $(MKDIR_P) "$(DESTDIR)$(pkgdatadir)/$$dir" || exit $$?;; \
	  f) \
	     if test "$$dir" = .; then dir=; else dir=/$$dir; fi; \
	     test -z "$$files" || { \
...
uninstall-nobase_pkgdataSCRIPTS:
	@$(NORMAL_UNINSTALL)
	@list='$(nobase_pkgdata_SCRIPTS)'; test -n "$(pkgdatadir)" || exit 0; \
	$(am__nobase_strip_setup); \
...
include udev/$(DEPDIR)/udev_udev_configure_printer-udev-configure-printer.Po
...
udev/udev_udev_configure_printer-udev-configure-printer.o: udev/udev-configure-printer.c
	...
	$(AM_V_at)$(am__mv) udev/$(DEPDIR)/udev_udev_configure_printer-udev-configure-printer.Tpo udev/$(DEPDIR)/udev_udev_configure_printer-udev-configure-printer.Po
#	$(AM_V_CC)source='udev/udev-configure-printer.c' object='udev/udev_udev_configure_printer-udev-configure-printer.o' libtool=no \
#	DEPDIR=$(DEPDIR) $(CCDEPMODE) $(depcomp) \
#	$(AM_V_CC_no)$(CC) $(DEFS) $(DEFAULT_INCLUDES) $(INCLUDES) $(AM_CPPFLAGS) $(CPPFLAGS) $(udev_udev_configure_printer_CFLAGS) $(CFLAGS) -c -o udev/udev_udev_configure_printer-udev-configure-printer.o `test -f 'udev/udev-configure-printer.c' || echo '$(srcdir)/'`udev/udev-configure-printer.c
...
udev/udev_udev_configure_printer-udev-configure-printer.obj: udev/udev-configure-printer.c
	...
	$(AM_V_at)$(am__mv) udev/$(DEPDIR)/udev_udev_configure_printer-udev-configure-printer.Tpo udev/$(DEPDIR)/udev_udev_configure_printer-udev-configure-printer.Po
#	$(AM_V_CC)source='udev/udev-configure-printer.c' object='udev/udev_udev_configure_printer-udev-configure-printer.obj' libtool=no \
#	DEPDIR=$(DEPDIR) $(CCDEPMODE) $(depcomp) \
...
install-cupshelpersDATA: $(cupshelpers_DATA)
	@$(NORMAL_INSTALL)
	@list='$(cupshelpers_DATA)'; test -n "$(cupshelpersdir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(cupshelpersdir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(cupshelpersdir)" || exit 1; \
	fi; \
	for p in $$list; do \
	  if test -f "$$p"; then d=; else d="$(srcdir)/"; fi; \
	  echo "$$d$$p"; \
	...
	while read files; do \
	  echo " $(INSTALL_DATA) $$files '$(DESTDIR)$(cupshelpersdir)'"; \
	  $(INSTALL_DATA) $$files "$(DESTDIR)$(cupshelpersdir)" || exit $$?; \
	done

uninstall-cupshelpersDATA:
	@$(NORMAL_UNINSTALL)
	@list='$(cupshelpers_DATA)'; test -n "$(cupshelpersdir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(cupshelpersdir)'; $(am__uninstall_files_from_dir)
...
install-dbusservicesDATA: $(dbusservices_DATA)
	@$(NORMAL_INSTALL)
	@list='$(dbusservices_DATA)'; test -n "$(dbusservicesdir)" || list=; \
	if test -n "$$list"; then \
...
recheck: all 
	...
	$(MAKE) $(AM_MAKEFLAGS) $(TEST_SUITE_LOG) \
	        ...
	        TEST_LOGS="$$log_list"; \
	exit $$?
test-xml-validity.sh.log: test-xml-validity.sh
	@p='test-xml-validity.sh'; \
	b='test-xml-validity.sh'; \
	$(am__check_pre) $(LOG_DRIVER) --test-name "$$f" \
	--log-file $$b.log --trs-file $$b.trs \
	$(am__common_driver_flags) $(AM_LOG_DRIVER_FLAGS) $(LOG_DRIVER_FLAGS) -- $(LOG_COMPILE) \
	"$$tst" $(AM_TESTS_FD_REDIRECT)
py.test.sh.log: py.test.sh
	@p='py.test.sh'; \
	b='py.test.sh'; \
	$(am__check_pre) $(LOG_DRIVER) --test-name "$$f" \
	--log-file $$b.log --trs-file $$b.trs \
...
installdirs-am:
	for dir in "$(DESTDIR)$(udevdir)" "$(DESTDIR)$(bindir)" "$(DESTDIR)$(pkgdatadir)" "$(DESTDIR)$(udevdir)" "$(DESTDIR)$(man1dir)" "$(DESTDIR)$(appdatadir)" "$(DESTDIR)$(cupshelpersdir)" "$(DESTDIR)$(dbusdir)" "$(DESTDIR)$(dbusinterfacesdir)" "$(DESTDIR)$(dbusservicesdir)" "$(DESTDIR)$(desktopdir)" "$(DESTDIR)$(pkgdatadir)" "$(DESTDIR)$(systemdsystemunitdir)" "$(DESTDIR)$(udevrulesdir)"; do \
...
install-data-am: install-appdataDATA install-cupshelpersDATA \
	...
	install-dbusservicesDATA install-desktopDATA install-man \
	install-nobase_pkgdataDATA install-nobase_pkgdataSCRIPTS \
...
uninstall-am: uninstall-appdataDATA uninstall-binSCRIPTS \
	uninstall-cupshelpersDATA uninstall-dbusDATA \
	uninstall-dbusinterfacesDATA uninstall-dbusservicesDATA \
...
.PHONY: $(am__recursive_targets) CTAGS GTAGS TAGS all all-am all-local \
	...
	install-binSCRIPTS install-cupshelpersDATA install-data \
	install-data-am install-dbusDATA install-dbusinterfacesDATA \
	install-dbusservicesDATA install-desktopDATA install-dvi \
	...
	uninstall-binSCRIPTS uninstall-cupshelpersDATA \
	uninstall-dbusDATA uninstall-dbusinterfacesDATA \
