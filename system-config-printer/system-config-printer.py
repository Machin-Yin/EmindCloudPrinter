...
class GUI(GtkGUI):
    ...
    def do_my_signal(self):
        subprocess.Popen("/home/wufeiyun/ProjectEx/build-EmindPrint-Desktop5_5-Release/EmindPrint")
...
#        
        ...
        #style window and subwidgets
        self.btnAddNew.connect('clicked',self.on_btnAddNew_clicked)
        color = Gdk.color_parse('white')
        self.dests_notebook.modify_bg(0,color)
        self.SelectPrinterWin.modify_bg(0,color)
        self.SelectPrinterWin.set_transient_for(self.PrintersWindow)
#         self.btnSelectCancel.connect('clicked',self.on_btnSelectCancel_clicked)
#         self.btnSelectNext.connect('clicked',self.on_btnSelectNext_clicked)
        
#         cssProvider = Gtk.CssProvider()
#         cssProvider.load_from_path('style.css')
        
        self.btnSelectLocal.get_style_context().add_provider(self.cssProvider,Gtk.STYLE_PROVIDER_PRIORITY_USER)
        self.btnSelectEmind.get_style_context().add_provider(self.cssProvider,Gtk.STYLE_PROVIDER_PRIORITY_USER)
        self.btnSelectNext.get_style_context().add_provider(self.cssProvider,Gtk.STYLE_PROVIDER_PRIORITY_USER)
        self.btnSelectCancel.get_style_context().add_provider(self.cssProvider,Gtk.STYLE_PROVIDER_PRIORITY_USER)
#         self.btnSelectCancel.get_style_context().add_provider(self.cssProvider,Gtk.STYLE_PROVIDER_PRIORITY_USER)

#         self.lbSelectTitle.get_style_context().add_provider(cssProvider,Gtk.STYLE_PROVIDER_PRIORITY_USER)
#         self.PrintersWindow.get_style_context().add_provider(cssProvider,Gtk.STYLE_PROVIDER_PRIORITY_USER)
#         self.SelectPrinterWin.get_style_context().add_provider(cssProvider,Gtk.STYLE_PROVIDER_PRIORITY_USER)

#         style_context = self.PrintersWindow.get_style_context()
#         style_context.add_provider(cssProvider,Gtk.STYLE_PROVIDER_PRIORITY_USER)
       
#         screen = Gdk.Screen.get_default()
#         styleContext = self.PrintersWindow.get_style_context()
#         styleContext.add_provider_for_screen(screen, cssProvider,Gtk.STYLE_PROVIDER_PRIORITY_USER)
        
        
#         self.apply_css(self.lbSelectTitle,cssProvider)
        
        textSelect0 = "<span font='16' foreground='black' ><b>Choose Printers Type ?</b></span>"
        textSelect1 = "<span font='16' foreground='black' >Add Local Printers\n</span>" \
        "<span font='12' foreground='grey'>Be Sure You Have Connected to The Network,\nOr Start the BlueTooth or Wireless Printers.</span>"
        textSelect2 = "<span font='16' foreground='black' >Add Emind Printers\n</span>" \
        "<span font='12' foreground='grey'>Be Sure You Have Connected to EmindPrinter WindowsNT Server \nAnd You Have Got The Authcode.</span>"
        self.lbSelectTitle.set_markup(textSelect0)
        self.btnSelectLocal.get_child().set_markup(textSelect1)
        self.btnSelectEmind.get_child().set_markup(textSelect2)
        self.SelectPrinterWin.connect ("delete-event",
                                       self.on_delSelectWin_hide)
...
#     def apply_css(self,widget,styleProvider):
#         widget.get_style_context().add_provider(styleProvider,Gtk.STYLE_PROVIDER_PRIORITY_USER)
        # new local or emind printer
    def on_delSelectWin_hide (self,widget, event):
        widget.hide()
        return False
        
    def on_btnAddNew_clicked(self,sensitive=True):
        self.SelectPrinterWin.set_title("Choose Printers")
        response = self.SelectPrinterWin.run()
        return
    def runExternal(self):
        subprocess.Popen("/home/wufeiyun/ProjectEx/build-EmindPrint-Desktop5_5-Release/EmindPrint")

    
    def on_btnSelectCancel_clicked(self, widget):
        """Close Connect dialog"""
        self.SelectPrinterWin.hide()
        
    def on_btnSelectNext_clicked(self,widget):
        if self.printerType == 1:  #local printer
            self.SelectPrinterWin.hide()
            self.emit("my_signal")
        else:
            self.on_new_printer_activate(self.btnAddFirstPrinter)
            self.SelectPrinterWin.hide()
                
    def on_btnSelectLocal_clicked(self,widget):
#         lbColor = Gdk.color_parse("#CCCCCC")
#         self.btnSelectLocal.modify_bg(0,lbColor)
        self.printerType = 0
        widget.get_style_context().add_class('btnSelectedLocal')
        widget.get_style_context().add_provider(self.cssProvider,Gtk.STYLE_PROVIDER_PRIORITY_USER)
        self.btnSelectEmind.get_style_context().add_class('btnSelectLocal')
        self.btnSelectEmind.get_style_context().add_provider(self.cssProvider,0)
        
    def on_btnSelectEmind_clicked(self,widget):
#         lbColor = Gdk.color_parse("#CCCCCC")
#         self.btnSelectEmind.modify_bg(0,lbColor)
#         cssProvider = Gtk.CssProvider()
#         cssProvider.load_from_path('style.css')
        self.printerType = 1
        widget.get_style_context().add_class('btnSelectedLocal')
        widget.get_style_context().add_provider(self.cssProvider,Gtk.STYLE_PROVIDER_PRIORITY_USER)
        self.btnSelectLocal.get_style_context().add_class('btnSelectLocal')
        self.btnSelectLocal.get_style_context().add_provider(self.cssProvider,0)
        
    def on_btnNextEmindPrint_clicked(self,name):
        emConn = cups.Connection();
    ...
    def polkit_permission_changed (self, widget, UNUSED):
        ...
        self.btnAddEprinter.set_sensitive(can_edit)
        self.btnAddNew.set_sensitive(can_edit)
    ...
    def populateList(self, prompt_allowed=True):
        ...
        if self.cups:
            if (not self.current_filter_text and
                ...
                #page = self.DESTS_PAGE_DESTS  # directly show EmindPrinter
